(function () {
 	Dope.UI.createProfileWindow = function (options) {
...
//			var closeButton = Ti.UI.createButton({title:"Close"});
	...
	// 	
		...
		var avatarURL = Defaults.HTTP_SERVER_NAME + Defaults.PROFILE_FOLDER + "/" + Profile.data.hash_id + "/avatar.jpg";
		Ti.API.log("AVATAR:" + avatarURL);
		...
		profileImage.addEventListener("click", function () {
			...
			Profile.updateAvatar({
				...
				success: function (blob) {
					Ti.API.log("avatar updated!");
					profileRow.profileImage.image = blob;
				},
				downloadStarted: function () {
					Ti.API.log("avatar download started!");
				}
			})
		})
...
//			win.rightNavButton = closeButton;
		...
		win.addEventListener("focus", function () {
			Ti.API.log("profile tab focused, updating form");
...
//			var f = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory ,'wiz1.jpg');
//			if (f.exists()) {	
			Ti.API.log(Profile.data.hash_id);
				
//				profileRow.profileImage.image = ;
//			}
		//	Profile.get();
		...
		var profileWindow = Dope.UI.createNavigationGroupStack(win);
